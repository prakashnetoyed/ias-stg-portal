import {
  ImageCropperComponent
} from "./chunk-CMZTRR6R.js";
import {
  DefaultValueAccessor,
  FormBuilder,
  FormControlName,
  FormGroupDirective,
  NgControlStatus,
  NgControlStatusGroup,
  ReactiveFormsModule,
  Validators,
  ɵNgNoValidate
} from "./chunk-YRHWIA2M.js";
import {
  UserService
} from "./chunk-UKQX5KCK.js";
import "./chunk-NZQ6EF6T.js";
import {
  ToastService
} from "./chunk-KC2EVZ25.js";
import {
  environment
} from "./chunk-WYW2S4QW.js";
import {
  Title
} from "./chunk-7VYRPV2J.js";
import {
  HttpClient,
  HttpHeaders
} from "./chunk-SFG4SZVN.js";
import {
  CommonModule,
  NgIf
} from "./chunk-G2HLXPCJ.js";
import {
  ɵsetClassDebugInfo,
  ɵɵStandaloneFeature,
  ɵɵadvance,
  ɵɵdefineComponent,
  ɵɵdirectiveInject,
  ɵɵelement,
  ɵɵelementEnd,
  ɵɵelementStart,
  ɵɵgetCurrentView,
  ɵɵlistener,
  ɵɵloadQuery,
  ɵɵnamespaceHTML,
  ɵɵnamespaceSVG,
  ɵɵnextContext,
  ɵɵproperty,
  ɵɵqueryRefresh,
  ɵɵresetView,
  ɵɵrestoreView,
  ɵɵsanitizeUrl,
  ɵɵtemplate,
  ɵɵtext,
  ɵɵtextInterpolate,
  ɵɵviewQuery
} from "./chunk-2RXJFVL4.js";
import "./chunk-ASLTLD6L.js";

// src/app/user-portal/pages/user-profile/user-profile.component.ts
var _c0 = ["successToast"];
var _c1 = ["errorToast"];
function UserProfileComponent_image_cropper_23_Template(rf, ctx) {
  if (rf & 1) {
    const _r2 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "image-cropper", 24);
    \u0275\u0275listener("imageCropped", function UserProfileComponent_image_cropper_23_Template_image_cropper_imageCropped_0_listener($event) {
      \u0275\u0275restoreView(_r2);
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.imageCropped($event));
    });
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275property("imageChangedEvent", ctx_r2.imageChangedEvent)("maintainAspectRatio", true)("aspectRatio", 1 / 1)("resizeToWidth", 200)("roundCropper", true)("backgroundColor", "rgba(0,0,0,0.6)");
  }
}
function UserProfileComponent_div_25_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 25)(1, "p", 26);
    \u0275\u0275text(2, "Live Preview");
    \u0275\u0275elementEnd();
    \u0275\u0275element(3, "img", 27);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance(3);
    \u0275\u0275property("src", ctx_r2.croppedImage, \u0275\u0275sanitizeUrl);
  }
}
function UserProfileComponent_div_26_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 25)(1, "p", 26);
    \u0275\u0275text(2, "Final Cropped Image");
    \u0275\u0275elementEnd();
    \u0275\u0275element(3, "img", 28);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance(3);
    \u0275\u0275property("src", ctx_r2.finalImage, \u0275\u0275sanitizeUrl);
  }
}
function UserProfileComponent_div_34__svg_svg_21_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(0, "svg", 67)(1, "g", 68);
    \u0275\u0275element(2, "path", 69);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "defs")(4, "clipPath", 70);
    \u0275\u0275element(5, "rect", 71);
    \u0275\u0275elementEnd()()();
  }
}
function UserProfileComponent_div_34__svg_svg_22_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(0, "svg", 67)(1, "g", 72);
    \u0275\u0275element(2, "path", 73);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "defs")(4, "clipPath", 74);
    \u0275\u0275element(5, "rect", 71);
    \u0275\u0275elementEnd()()();
  }
}
function UserProfileComponent_div_34_div_23_span_1_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275text(1, "Password is required.");
    \u0275\u0275elementEnd();
  }
}
function UserProfileComponent_div_34_div_23_span_2_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275text(1, "Minimum 8 characters required.");
    \u0275\u0275elementEnd();
  }
}
function UserProfileComponent_div_34_div_23_span_3_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "span");
    \u0275\u0275text(1, " Must include uppercase, number, and special character. ");
    \u0275\u0275elementEnd();
  }
}
function UserProfileComponent_div_34_div_23_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 75);
    \u0275\u0275template(1, UserProfileComponent_div_34_div_23_span_1_Template, 2, 0, "span", 76)(2, UserProfileComponent_div_34_div_23_span_2_Template, 2, 0, "span", 76)(3, UserProfileComponent_div_34_div_23_span_3_Template, 2, 0, "span", 76);
    \u0275\u0275elementEnd();
  }
  if (rf & 2) {
    let tmp_4_0;
    let tmp_5_0;
    let tmp_6_0;
    const ctx_r2 = \u0275\u0275nextContext(2);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", (tmp_4_0 = ctx_r2.changepassword.get("newPassword")) == null ? null : tmp_4_0.errors == null ? null : tmp_4_0.errors["required"]);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", (tmp_5_0 = ctx_r2.changepassword.get("newPassword")) == null ? null : tmp_5_0.errors == null ? null : tmp_5_0.errors["minlength"]);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", (tmp_6_0 = ctx_r2.changepassword.get("newPassword")) == null ? null : tmp_6_0.errors == null ? null : tmp_6_0.errors["pattern"]);
  }
}
function UserProfileComponent_div_34__svg_svg_30_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(0, "svg", 67)(1, "g", 68);
    \u0275\u0275element(2, "path", 69);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "defs")(4, "clipPath", 70);
    \u0275\u0275element(5, "rect", 71);
    \u0275\u0275elementEnd()()();
  }
}
function UserProfileComponent_div_34__svg_svg_31_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(0, "svg", 67)(1, "g", 72);
    \u0275\u0275element(2, "path", 73);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "defs")(4, "clipPath", 74);
    \u0275\u0275element(5, "rect", 71);
    \u0275\u0275elementEnd()()();
  }
}
function UserProfileComponent_div_34_div_32_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 75);
    \u0275\u0275text(1, " Passwords do not match. ");
    \u0275\u0275elementEnd();
  }
}
function UserProfileComponent_div_34_div_84_div_4_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 64)(1, "span");
    \u0275\u0275text(2, "State: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "h5");
    \u0275\u0275text(4);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext(3);
    \u0275\u0275advance(4);
    \u0275\u0275textInterpolate((ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.placeOfDomicile) || "Not Provided");
  }
}
function UserProfileComponent_div_34_div_84_Template(rf, ctx) {
  if (rf & 1) {
    \u0275\u0275elementStart(0, "div", 62)(1, "h2");
    \u0275\u0275text(2, "Place Of Domicile");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(3, "div", 63);
    \u0275\u0275template(4, UserProfileComponent_div_34_div_84_div_4_Template, 5, 1, "div", 77);
    \u0275\u0275elementEnd()();
  }
  if (rf & 2) {
    const ctx_r2 = \u0275\u0275nextContext(2);
    \u0275\u0275advance(4);
    \u0275\u0275property("ngIf", ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.placeOfDomicile);
  }
}
function UserProfileComponent_div_34_Template(rf, ctx) {
  if (rf & 1) {
    const _r4 = \u0275\u0275getCurrentView();
    \u0275\u0275elementStart(0, "div", 29)(1, "div", 30)(2, "h1");
    \u0275\u0275text(3, "Profile");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(4, "div", 31)(5, "button", 32);
    \u0275\u0275text(6, "Change password");
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(7, "div", 33)(8, "div", 34)(9, "h5", 35);
    \u0275\u0275text(10, "Change Password");
    \u0275\u0275elementEnd();
    \u0275\u0275element(11, "button", 36);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(12, "div", 37)(13, "form", 38);
    \u0275\u0275listener("ngSubmit", function UserProfileComponent_div_34_Template_form_ngSubmit_13_listener() {
      \u0275\u0275restoreView(_r4);
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.onSubmit());
    });
    \u0275\u0275elementStart(14, "div", 39)(15, "div", 40)(16, "label", 41);
    \u0275\u0275text(17, "New Password");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(18, "div", 42);
    \u0275\u0275element(19, "input", 43);
    \u0275\u0275elementStart(20, "button", 44);
    \u0275\u0275listener("click", function UserProfileComponent_div_34_Template_button_click_20_listener() {
      \u0275\u0275restoreView(_r4);
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.toggleNewPassword());
    });
    \u0275\u0275template(21, UserProfileComponent_div_34__svg_svg_21_Template, 6, 0, "svg", 45)(22, UserProfileComponent_div_34__svg_svg_22_Template, 6, 0, "svg", 45);
    \u0275\u0275elementEnd()();
    \u0275\u0275template(23, UserProfileComponent_div_34_div_23_Template, 4, 3, "div", 46);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(24, "div", 40)(25, "label", 41);
    \u0275\u0275text(26, "Confirm Password");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(27, "div", 42);
    \u0275\u0275element(28, "input", 47);
    \u0275\u0275elementStart(29, "button", 44);
    \u0275\u0275listener("click", function UserProfileComponent_div_34_Template_button_click_29_listener() {
      \u0275\u0275restoreView(_r4);
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.toggleConfirmPassword());
    });
    \u0275\u0275template(30, UserProfileComponent_div_34__svg_svg_30_Template, 6, 0, "svg", 45)(31, UserProfileComponent_div_34__svg_svg_31_Template, 6, 0, "svg", 45);
    \u0275\u0275elementEnd()();
    \u0275\u0275template(32, UserProfileComponent_div_34_div_32_Template, 2, 0, "div", 46);
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(33, "div", 48);
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(34, "svg", 49);
    \u0275\u0275element(35, "path", 50);
    \u0275\u0275elementEnd();
    \u0275\u0275namespaceHTML();
    \u0275\u0275elementStart(36, "p");
    \u0275\u0275text(37, " Once your password is changed, you will need to log in again with the new password. ");
    \u0275\u0275elementEnd()()();
    \u0275\u0275elementStart(38, "div", 51)(39, "button", 52);
    \u0275\u0275text(40, "Back");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(41, "button", 53);
    \u0275\u0275text(42, " Submit ");
    \u0275\u0275elementEnd()()()()();
    \u0275\u0275elementStart(43, "div", 54)(44, "div", 55)(45, "div", 56);
    \u0275\u0275element(46, "img", 57);
    \u0275\u0275elementStart(47, "button", 58);
    \u0275\u0275listener("click", function UserProfileComponent_div_34_Template_button_click_47_listener() {
      \u0275\u0275restoreView(_r4);
      const ctx_r2 = \u0275\u0275nextContext();
      return \u0275\u0275resetView(ctx_r2.openModal());
    });
    \u0275\u0275namespaceSVG();
    \u0275\u0275elementStart(48, "svg", 59);
    \u0275\u0275element(49, "path", 60);
    \u0275\u0275elementEnd()()()();
    \u0275\u0275namespaceHTML();
    \u0275\u0275elementStart(50, "div", 61)(51, "div", 62)(52, "h2");
    \u0275\u0275text(53, "Personal Information");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(54, "div", 63)(55, "div", 64)(56, "span");
    \u0275\u0275text(57, "Name: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(58, "h5");
    \u0275\u0275text(59);
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(60, "div", 64)(61, "span");
    \u0275\u0275text(62, "Gender: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(63, "h5");
    \u0275\u0275text(64);
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(65, "div", 64)(66, "span");
    \u0275\u0275text(67, "DOB: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(68, "h5");
    \u0275\u0275text(69);
    \u0275\u0275elementEnd()()()();
    \u0275\u0275elementStart(70, "div", 62)(71, "h2");
    \u0275\u0275text(72, "Contact Details");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(73, "div", 63)(74, "div", 65)(75, "span");
    \u0275\u0275text(76, "E- Mail: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(77, "h5");
    \u0275\u0275text(78);
    \u0275\u0275elementEnd()();
    \u0275\u0275elementStart(79, "div", 64)(80, "span");
    \u0275\u0275text(81, "Mobile No: ");
    \u0275\u0275elementEnd();
    \u0275\u0275elementStart(82, "h5");
    \u0275\u0275text(83);
    \u0275\u0275elementEnd()()()();
    \u0275\u0275template(84, UserProfileComponent_div_34_div_84_Template, 5, 1, "div", 66);
    \u0275\u0275elementEnd()()();
  }
  if (rf & 2) {
    let tmp_7_0;
    let tmp_11_0;
    const ctx_r2 = \u0275\u0275nextContext();
    \u0275\u0275advance(13);
    \u0275\u0275property("formGroup", ctx_r2.changepassword);
    \u0275\u0275advance(6);
    \u0275\u0275property("type", ctx_r2.showNewPassword ? "text" : "password");
    \u0275\u0275advance(2);
    \u0275\u0275property("ngIf", !ctx_r2.showNewPassword);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", ctx_r2.showNewPassword);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", ((tmp_7_0 = ctx_r2.changepassword.get("newPassword")) == null ? null : tmp_7_0.touched) && ((tmp_7_0 = ctx_r2.changepassword.get("newPassword")) == null ? null : tmp_7_0.invalid));
    \u0275\u0275advance(5);
    \u0275\u0275property("type", ctx_r2.showConfirmPassword ? "text" : "password");
    \u0275\u0275advance(2);
    \u0275\u0275property("ngIf", !ctx_r2.showConfirmPassword);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", ctx_r2.showConfirmPassword);
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", ctx_r2.changepassword.hasError("passwordMismatch") && ((tmp_11_0 = ctx_r2.changepassword.get("confirmPassword")) == null ? null : tmp_11_0.touched));
    \u0275\u0275advance(9);
    \u0275\u0275property("disabled", ctx_r2.changepassword.invalid);
    \u0275\u0275advance(5);
    \u0275\u0275property("src", (ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.profilePic) || "assets/images/avatar.png", \u0275\u0275sanitizeUrl);
    \u0275\u0275advance(13);
    \u0275\u0275textInterpolate(ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.englishName);
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate((ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.gender) || "N/A");
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.dob);
    \u0275\u0275advance(9);
    \u0275\u0275textInterpolate(ctx_r2.user.email || "Not provided");
    \u0275\u0275advance(5);
    \u0275\u0275textInterpolate(ctx_r2.user.mobileNumber || "Not Provided");
    \u0275\u0275advance();
    \u0275\u0275property("ngIf", ctx_r2.user.doptOfficer == null ? null : ctx_r2.user.doptOfficer.placeOfDomicile);
  }
}
var UserProfileComponent = class _UserProfileComponent {
  constructor(fb, userService, http, toastService, titleService) {
    this.fb = fb;
    this.userService = userService;
    this.http = http;
    this.toastService = toastService;
    this.titleService = titleService;
    this.user = null;
    this.showNewPassword = false;
    this.showConfirmPassword = false;
    this.imageChangedEvent = "";
    this.croppedImage = null;
    this.finalImage = null;
    this.passwordsMatchValidator = (group) => {
      const newPassword = group.get("newPassword")?.value;
      const confirmPassword = group.get("confirmPassword")?.value;
      return newPassword === confirmPassword ? null : { passwordMismatch: true };
    };
  }
  ngOnInit() {
    this.titleService.setTitle("IASCA Portal - User Profile");
    this.userService.getUser().subscribe((user) => {
      this.user = user;
    });
    this.changepassword = this.fb.group({
      newPassword: [
        "",
        [
          Validators.required,
          Validators.minLength(8),
          Validators.pattern(/^(?=.*[A-Z])(?=.*[a-z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/)
        ]
      ],
      confirmPassword: ["", Validators.required]
    }, { validators: this.passwordsMatchValidator });
  }
  showSuccessToast() {
    const toast = new bootstrap.Toast(this.successToast.nativeElement);
    toast.show();
  }
  showErrorToast() {
    const toast = new bootstrap.Toast(this.errorToast.nativeElement);
    toast.show();
  }
  onSubmit() {
    if (this.changepassword.invalid) {
      this.showErrorToast();
      return;
    }
    const { newPassword, confirmPassword } = this.changepassword.value;
    const token = localStorage.getItem("authToken");
    const headers = new HttpHeaders({
      Authorization: `Bearer ${token}`
    });
    const body = {
      password: newPassword,
      confirmPassword
    };
    this.http.post(`${environment.apiUrl}/accounts/setup-password`, body, { headers }).subscribe({
      next: () => {
        this.showSuccessToast();
        setTimeout(() => {
          localStorage.clear();
          window.location.href = "";
        }, 2e3);
      },
      error: (err) => {
        console.error("Error setting up password:", err);
        this.showErrorToast();
      }
    });
  }
  toggleNewPassword() {
    this.showNewPassword = !this.showNewPassword;
  }
  toggleConfirmPassword() {
    this.showConfirmPassword = !this.showConfirmPassword;
  }
  openModal() {
    const modalEl = document.getElementById("uploadModal");
    if (modalEl) {
      this.modalInstance = new bootstrap.Modal(modalEl);
      this.modalInstance.show();
    }
  }
  onFileChange(event) {
    this.imageChangedEvent = event;
    this.croppedImage = null;
    this.finalImage = null;
  }
  // Convert Blob to Base64
  blobToBase64(blob) {
    return new Promise((resolve, reject) => {
      const reader = new FileReader();
      reader.onloadend = () => resolve(reader.result);
      reader.onerror = reject;
      reader.readAsDataURL(blob);
    });
  }
  imageCropped(event) {
    if (event.blob) {
      this.blobToBase64(event.blob).then((base64) => this.croppedImage = base64).catch((err) => console.error("Blob to Base64 conversion error:", err));
    }
  }
  // Lock the cropped Base64 image
  cropImage() {
    this.finalImage = this.croppedImage;
  }
  // Send Base64 image to API and refresh user from server
  saveImage() {
    if (!this.finalImage)
      return;
    const body = { profilePic: this.finalImage };
    this.http.patch(`${environment.apiUrl}/accounts/me/profile-pic`, body).subscribe({
      next: () => {
        this.userService.refreshUser().subscribe((u) => {
          this.user = u;
        });
        this.toastService.showToast("Profile Pic Updated", "success");
        this.modalInstance.hide();
      },
      error: (err) => {
        console.error(err);
        this.showErrorToast();
      }
    });
  }
  static {
    this.\u0275fac = function UserProfileComponent_Factory(t) {
      return new (t || _UserProfileComponent)(\u0275\u0275directiveInject(FormBuilder), \u0275\u0275directiveInject(UserService), \u0275\u0275directiveInject(HttpClient), \u0275\u0275directiveInject(ToastService), \u0275\u0275directiveInject(Title));
    };
  }
  static {
    this.\u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _UserProfileComponent, selectors: [["app-user-profile"]], viewQuery: function UserProfileComponent_Query(rf, ctx) {
      if (rf & 1) {
        \u0275\u0275viewQuery(_c0, 7);
        \u0275\u0275viewQuery(_c1, 7);
      }
      if (rf & 2) {
        let _t;
        \u0275\u0275queryRefresh(_t = \u0275\u0275loadQuery()) && (ctx.successToast = _t.first);
        \u0275\u0275queryRefresh(_t = \u0275\u0275loadQuery()) && (ctx.errorToast = _t.first);
      }
    }, standalone: true, features: [\u0275\u0275StandaloneFeature], decls: 35, vars: 6, consts: [["successToast", ""], ["errorToast", ""], [1, "toast-container", "position-fixed", "top-0", "end-0", "p-3"], ["role", "alert", "aria-live", "assertive", "aria-atomic", "true", 1, "toast", "align-items-center", "text-bg-success", "border-0"], [1, "d-flex"], [1, "toast-body"], ["type", "button", "data-bs-dismiss", "toast", 1, "btn-close", "btn-close-white", "me-2", "m-auto"], ["role", "alert", "aria-live", "assertive", "aria-atomic", "true", 1, "toast", "align-items-center", "text-bg-danger", "border-0"], ["id", "uploadModal", "tabindex", "-1", "aria-hidden", "true", 1, "modal", "fade"], [1, "modal-dialog", "modal-dialog-centered"], [1, "modal-content", "p-3"], [1, "modal-header"], [1, "modal-title"], ["type", "button", "data-bs-dismiss", "modal", 1, "btn-close"], [1, "modal-body"], ["type", "file", "accept", "image/*", 1, "form-control", "mb-3", 3, "change"], ["format", "png", 3, "imageChangedEvent", "maintainAspectRatio", "aspectRatio", "resizeToWidth", "roundCropper", "backgroundColor", "imageCropped", 4, "ngIf"], [1, "preview", "d-flex", "align-items-center", "justify-content-between"], ["class", "text-center mt-3", 4, "ngIf"], [1, "modal-footer", "d-flex", "justify-content-between"], ["data-bs-dismiss", "modal", 1, "btn", "btn-secondary"], [1, "btn", "btn-dark", 3, "click", "disabled"], [1, "btn", "btn-prime", 3, "click", "disabled"], ["class", "main-profile-info", 4, "ngIf"], ["format", "png", 3, "imageCropped", "imageChangedEvent", "maintainAspectRatio", "aspectRatio", "resizeToWidth", "roundCropper", "backgroundColor"], [1, "text-center", "mt-3"], [1, "small", "text-muted", "mb-1"], [1, "rounded-circle", "preview-img", 3, "src"], [1, "rounded-circle", "preview-img", "border", "border-2", 3, "src"], [1, "main-profile-info"], [1, "main-page-heading", "d-flex", "align-items-center", "justify-content-between", "mb-4"], [1, "buttonss", "d-flex", "gap-3"], ["data-bs-toggle", "offcanvas", "data-bs-target", "#addNewsDrawer", 1, "btn", "btn-donate"], ["tabindex", "-1", "id", "addNewsDrawer", "aria-labelledby", "addNewsDrawerLabel", 1, "offcanvas", "offcanvas-end"], [1, "offcanvas-header"], ["id", "addNewsDrawerLabel"], ["type", "button", "data-bs-dismiss", "offcanvas", "aria-label", "Close", 1, "btn-close", "text-reset"], [1, "offcanvas-body"], [1, "d-flex", "flex-column", "justify-content-between", "h-100", 3, "ngSubmit", "formGroup"], [1, "inner-fields"], [1, "mb-3", "position-relative"], [1, "form-label"], [1, "password-input"], ["placeholder", "Enter new password", "formControlName", "newPassword", 1, "form-control", 3, "type"], ["type", "button", 1, "eye-button", 3, "click"], ["xmlns", "http://www.w3.org/2000/svg", "width", "19", "height", "19", "viewBox", "0 0 19 19", "fill", "none", 4, "ngIf"], ["class", "text-danger small mt-1", 4, "ngIf"], ["placeholder", "Re-enter new password", "formControlName", "confirmPassword", 1, "form-control", 3, "type"], [1, "warning-box"], ["xmlns", "http://www.w3.org/2000/svg", "width", "14", "height", "14", "viewBox", "0 0 14 14", "fill", "none"], ["d", "M6.33325 10.3333H7.66658V6.33331H6.33325V10.3333ZM6.99992 4.99998C7.18881 4.99998 7.34714 4.93609 7.47492 4.80831C7.6027 4.68053 7.66658 4.5222 7.66658 4.33331C7.66658 4.14442 7.6027 3.98609 7.47492 3.85831C7.34714 3.73054 7.18881 3.66665 6.99992 3.66665C6.81103 3.66665 6.6527 3.73054 6.52492 3.85831C6.39714 3.98609 6.33325 4.14442 6.33325 4.33331C6.33325 4.5222 6.39714 4.68053 6.52492 4.80831C6.6527 4.93609 6.81103 4.99998 6.99992 4.99998ZM6.99992 13.6666C6.0777 13.6666 5.21103 13.4916 4.39992 13.1416C3.58881 12.7916 2.88325 12.3166 2.28325 11.7166C1.68325 11.1166 1.20825 10.4111 0.858252 9.59998C0.508252 8.78887 0.333252 7.9222 0.333252 6.99998C0.333252 6.07776 0.508252 5.21109 0.858252 4.39998C1.20825 3.58887 1.68325 2.88331 2.28325 2.28331C2.88325 1.68331 3.58881 1.20831 4.39992 0.858313C5.21103 0.508313 6.0777 0.333313 6.99992 0.333313C7.92214 0.333313 8.78881 0.508313 9.59992 0.858313C10.411 1.20831 11.1166 1.68331 11.7166 2.28331C12.3166 2.88331 12.7916 3.58887 13.1416 4.39998C13.4916 5.21109 13.6666 6.07776 13.6666 6.99998C13.6666 7.9222 13.4916 8.78887 13.1416 9.59998C12.7916 10.4111 12.3166 11.1166 11.7166 11.7166C11.1166 12.3166 10.411 12.7916 9.59992 13.1416C8.78881 13.4916 7.92214 13.6666 6.99992 13.6666ZM6.99992 12.3333C8.48881 12.3333 9.74992 11.8166 10.7833 10.7833C11.8166 9.74998 12.3333 8.48887 12.3333 6.99998C12.3333 5.51109 11.8166 4.24998 10.7833 3.21665C9.74992 2.18331 8.48881 1.66665 6.99992 1.66665C5.51103 1.66665 4.24992 2.18331 3.21659 3.21665C2.18325 4.24998 1.66659 5.51109 1.66659 6.99998C1.66659 8.48887 2.18325 9.74998 3.21659 10.7833C4.24992 11.8166 5.51103 12.3333 6.99992 12.3333Z", "fill", "#A16800"], [1, "d-flex", "justify-content-end", "gap-3", "mt-3"], ["type", "button", "data-bs-dismiss", "offcanvas", 1, "btn", "btn-outline-secondary"], ["type", "submit", 1, "btn", "btn-prime", 3, "disabled"], [1, "all-user-deatils-div"], [1, "profile-photo-div", "p-2"], [1, "inner-profile-div", "position-relative"], ["alt", "profile-image", "draggable", "false", 1, "profile-image", 3, "src"], [1, "edit-icon-btn", "position-absolute", 3, "click"], ["xmlns", "http://www.w3.org/2000/svg", "width", "16", "height", "16", "viewBox", "0 0 16 16", "fill", "none"], ["d", "M2.16667 13.8333H3.35417L11.5 5.6875L10.3125 4.5L2.16667 12.6458V13.8333ZM0.5 15.5V11.9583L11.5 0.979167C11.6667 0.826389 11.8507 0.708333 12.0521 0.625C12.2535 0.541667 12.4653 0.5 12.6875 0.5C12.9097 0.5 13.125 0.541667 13.3333 0.625C13.5417 0.708333 13.7222 0.833333 13.875 1L15.0208 2.16667C15.1875 2.31944 15.309 2.5 15.3854 2.70833C15.4618 2.91667 15.5 3.125 15.5 3.33333C15.5 3.55556 15.4618 3.76736 15.3854 3.96875C15.309 4.17014 15.1875 4.35417 15.0208 4.52083L4.04167 15.5H0.5ZM10.8958 5.10417L10.3125 4.5L11.5 5.6875L10.8958 5.10417Z", "fill", "#211B3A"], [1, "info-div"], [1, "info-heading-div"], [1, "inner-info", "d-flex", "flex-column", "gap-3"], [1, "inf"], [1, "inf", "d-flex", "align-items-center"], ["class", "info-heading-div", 4, "ngIf"], ["xmlns", "http://www.w3.org/2000/svg", "width", "19", "height", "19", "viewBox", "0 0 19 19", "fill", "none"], ["clip-path", "url(#clip0_50828_258)"], ["d", "M9.49996 4.75002C12.5004 4.75002 15.1762 6.43627 16.4825 9.10419C15.1762 11.7721 12.5004 13.4584 9.49996 13.4584C6.49954 13.4584 3.82371 11.7721 2.51746 9.10419C3.82371 6.43627 6.49954 4.75002 9.49996 4.75002ZM9.49996 3.16669C5.54163 3.16669 2.16121 5.62877 0.791626 9.10419C2.16121 12.5796 5.54163 15.0417 9.49996 15.0417C13.4583 15.0417 16.8387 12.5796 18.2083 9.10419C16.8387 5.62877 13.4583 3.16669 9.49996 3.16669ZM9.49996 7.12502C10.5925 7.12502 11.4791 8.01169 11.4791 9.10419C11.4791 10.1967 10.5925 11.0834 9.49996 11.0834C8.40746 11.0834 7.52079 10.1967 7.52079 9.10419C7.52079 8.01169 8.40746 7.12502 9.49996 7.12502ZM9.49996 5.54169C7.53663 5.54169 5.93746 7.14085 5.93746 9.10419C5.93746 11.0675 7.53663 12.6667 9.49996 12.6667C11.4633 12.6667 13.0625 11.0675 13.0625 9.10419C13.0625 7.14085 11.4633 5.54169 9.49996 5.54169Z", "fill", "#A9ACB1"], ["id", "clip0_50828_258"], ["width", "19", "height", "19", "fill", "white"], ["clip-path", "url(#clip0_50828_300)"], ["d", "M9.50008 4.74999C12.5005 4.74999 15.1763 6.43624 16.4826 9.10416C16.0155 10.07 15.3584 10.9012 14.5747 11.5742L15.6909 12.6904C16.7913 11.7167 17.6622 10.4975 18.2084 9.10416C16.8388 5.62874 13.4584 3.16666 9.50008 3.16666C8.49467 3.16666 7.52883 3.32499 6.61841 3.61791L7.92467 4.92416C8.43925 4.82124 8.96175 4.74999 9.50008 4.74999ZM8.653 5.65249L10.2917 7.29124C10.743 7.48916 11.1072 7.85333 11.3051 8.30458L12.9438 9.94333C13.0072 9.67416 13.0547 9.38916 13.0547 9.09624C13.0626 7.13291 11.4634 5.54166 9.50008 5.54166C9.20717 5.54166 8.93008 5.58124 8.653 5.65249ZM1.59133 3.06374L3.713 5.18541C2.42258 6.19874 1.40133 7.54458 0.791748 9.10416C2.16133 12.5796 5.54175 15.0417 9.50008 15.0417C10.7034 15.0417 11.8592 14.8121 12.9201 14.3925L15.6276 17.1L16.7438 15.9837L2.70758 1.93958L1.59133 3.06374ZM7.52883 9.00124L9.59508 11.0675C9.56342 11.0754 9.53175 11.0833 9.50008 11.0833C8.40758 11.0833 7.52092 10.1967 7.52092 9.10416C7.52092 9.06458 7.52883 9.04083 7.52883 9.00124ZM4.83717 6.30958L6.22258 7.69499C6.0405 8.13041 5.93758 8.60541 5.93758 9.10416C5.93758 11.0675 7.53675 12.6667 9.50008 12.6667C9.99883 12.6667 10.4738 12.5637 10.9013 12.3817L11.6772 13.1575C10.9805 13.3475 10.2522 13.4583 9.50008 13.4583C6.49967 13.4583 3.82383 11.7721 2.51758 9.10416C3.07175 7.97208 3.87925 7.03791 4.83717 6.30958Z", "fill", "#222222"], ["id", "clip0_50828_300"], [1, "text-danger", "small", "mt-1"], [4, "ngIf"], ["class", "inf", 4, "ngIf"]], template: function UserProfileComponent_Template(rf, ctx) {
      if (rf & 1) {
        const _r1 = \u0275\u0275getCurrentView();
        \u0275\u0275elementStart(0, "div", 2)(1, "div", 3, 0)(3, "div", 4)(4, "div", 5);
        \u0275\u0275text(5, " \u2705 Password changed successfully. Logging out... ");
        \u0275\u0275elementEnd();
        \u0275\u0275element(6, "button", 6);
        \u0275\u0275elementEnd()()();
        \u0275\u0275elementStart(7, "div", 2)(8, "div", 7, 1)(10, "div", 4)(11, "div", 5);
        \u0275\u0275text(12, " \u274C Failed to change password. Please try again. ");
        \u0275\u0275elementEnd();
        \u0275\u0275element(13, "button", 6);
        \u0275\u0275elementEnd()()();
        \u0275\u0275elementStart(14, "div", 8)(15, "div", 9)(16, "div", 10)(17, "div", 11)(18, "h5", 12);
        \u0275\u0275text(19, "Upload & Crop Image");
        \u0275\u0275elementEnd();
        \u0275\u0275element(20, "button", 13);
        \u0275\u0275elementEnd();
        \u0275\u0275elementStart(21, "div", 14)(22, "input", 15);
        \u0275\u0275listener("change", function UserProfileComponent_Template_input_change_22_listener($event) {
          \u0275\u0275restoreView(_r1);
          return \u0275\u0275resetView(ctx.onFileChange($event));
        });
        \u0275\u0275elementEnd();
        \u0275\u0275template(23, UserProfileComponent_image_cropper_23_Template, 1, 6, "image-cropper", 16);
        \u0275\u0275elementStart(24, "div", 17);
        \u0275\u0275template(25, UserProfileComponent_div_25_Template, 4, 1, "div", 18)(26, UserProfileComponent_div_26_Template, 4, 1, "div", 18);
        \u0275\u0275elementEnd()();
        \u0275\u0275elementStart(27, "div", 19)(28, "button", 20);
        \u0275\u0275text(29, "Cancel");
        \u0275\u0275elementEnd();
        \u0275\u0275elementStart(30, "button", 21);
        \u0275\u0275listener("click", function UserProfileComponent_Template_button_click_30_listener() {
          \u0275\u0275restoreView(_r1);
          return \u0275\u0275resetView(ctx.cropImage());
        });
        \u0275\u0275text(31, "Crop");
        \u0275\u0275elementEnd();
        \u0275\u0275elementStart(32, "button", 22);
        \u0275\u0275listener("click", function UserProfileComponent_Template_button_click_32_listener() {
          \u0275\u0275restoreView(_r1);
          return \u0275\u0275resetView(ctx.saveImage());
        });
        \u0275\u0275text(33, "Save");
        \u0275\u0275elementEnd()()()()();
        \u0275\u0275template(34, UserProfileComponent_div_34_Template, 85, 17, "div", 23);
      }
      if (rf & 2) {
        \u0275\u0275advance(23);
        \u0275\u0275property("ngIf", ctx.imageChangedEvent);
        \u0275\u0275advance(2);
        \u0275\u0275property("ngIf", ctx.croppedImage);
        \u0275\u0275advance();
        \u0275\u0275property("ngIf", ctx.finalImage);
        \u0275\u0275advance(4);
        \u0275\u0275property("disabled", !ctx.croppedImage);
        \u0275\u0275advance(2);
        \u0275\u0275property("disabled", !ctx.finalImage);
        \u0275\u0275advance(2);
        \u0275\u0275property("ngIf", ctx.user);
      }
    }, dependencies: [CommonModule, NgIf, ReactiveFormsModule, \u0275NgNoValidate, DefaultValueAccessor, NgControlStatus, NgControlStatusGroup, FormGroupDirective, FormControlName, ImageCropperComponent], styles: ["\n\n.main-profile-info[_ngcontent-%COMP%]   h1[_ngcontent-%COMP%] {\n  color: var(--Text, #211B3A);\n  font-family: Urbanist;\n  font-size: 24px;\n  font-style: normal;\n  font-weight: 600;\n  line-height: 28px;\n}\n.main-profile-info[_ngcontent-%COMP%]   .all-user-deatils-div[_ngcontent-%COMP%] {\n  border-radius: 12px;\n  background: #FFF;\n  padding: 120px 30px 30px 30px;\n  margin-top: 90px;\n  position: relative;\n}\n.info-div[_ngcontent-%COMP%] {\n  display: grid;\n  grid-template-columns: repeat(3, 1fr);\n}\n.info-heading-div[_ngcontent-%COMP%]   h2[_ngcontent-%COMP%] {\n  color: var(--Text, #211B3A);\n  font-family: Urbanist;\n  font-size: 22px;\n  font-style: normal;\n  font-weight: 600;\n  line-height: 28px;\n  margin-bottom: 30px;\n}\n.inf[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n}\n.inf[_ngcontent-%COMP%]   span[_ngcontent-%COMP%] {\n  color: #000;\n  font-family: Urbanist;\n  font-size: 18px;\n  font-style: normal;\n  font-weight: 500;\n  line-height: 28px;\n  width: 100px;\n}\n.inf[_ngcontent-%COMP%]   h5[_ngcontent-%COMP%] {\n  color: rgba(0, 0, 0, 0.78);\n  font-family: Urbanist;\n  font-size: 18px;\n  font-style: normal;\n  font-weight: 400;\n  line-height: 28px;\n  margin: 0px;\n}\n.inf[_ngcontent-%COMP%]   h6[_ngcontent-%COMP%] {\n  color: rgba(11, 56, 161, 0.78);\n  font-family: Urbanist;\n  font-size: 18px;\n  font-style: normal;\n  font-weight: 400;\n  line-height: 28px;\n  margin: 0px;\n  cursor: pointer;\n}\n.warning-box[_ngcontent-%COMP%] {\n  display: flex;\n  align-items: center;\n  background-color: #fff4db;\n  border-radius: 8px;\n  padding: 12px;\n  gap: 8px;\n  color: #333;\n}\n.warning-box[_ngcontent-%COMP%]   p[_ngcontent-%COMP%] {\n  color: #A16800;\n  font-family: Urbanist;\n  font-size: 12px;\n  font-style: normal;\n  font-weight: 500;\n  line-height: 18px;\n  margin: 0;\n}\n.form-label[_ngcontent-%COMP%] {\n  color: var(--Text-Dark, #212121);\n  font-family: Urbanist;\n  font-size: var(--Font-Size-2, 14px);\n  font-style: normal;\n  font-weight: 500;\n  line-height: var(--Line-Heights-10, 20px);\n  letter-spacing: var(--Letter-Spacing-2, 0.1px);\n}\n.password-input[_ngcontent-%COMP%] {\n  position: relative;\n}\n.password-input[_ngcontent-%COMP%]   input[_ngcontent-%COMP%] {\n  width: 100%;\n  padding-right: 40px;\n}\n.password-input[_ngcontent-%COMP%]   .eye-button[_ngcontent-%COMP%] {\n  position: absolute;\n  right: 10px;\n  top: 50%;\n  transform: translateY(-50%);\n  background: transparent;\n  border: none;\n  cursor: pointer;\n  padding: 0;\n}\n.password-input[_ngcontent-%COMP%]   .eye-button[_ngcontent-%COMP%]   img[_ngcontent-%COMP%] {\n  width: 20px;\n}\n.profile-photo-div[_ngcontent-%COMP%] {\n  border-radius: 999px;\n  box-shadow: 1px -23px 29px -13px rgba(0, 0, 0, 0.216);\n  width: max-content;\n  position: absolute;\n  top: -50%;\n  left: 50%;\n  transform: translate(-50%, 50%);\n}\n.profile-photo-div[_ngcontent-%COMP%]   img[_ngcontent-%COMP%] {\n  border-radius: 999px;\n  height: 150px;\n  width: 150px;\n}\n.profile-image[_ngcontent-%COMP%] {\n  width: 100%;\n  height: 100%;\n  object-fit: cover;\n}\n.edit-icon-btn[_ngcontent-%COMP%] {\n  position: absolute;\n  bottom: 4px;\n  right: -10px;\n  background: rgba(200, 200, 200, 0.6);\n  color: #fff;\n  border: none;\n  border-radius: 50%;\n  height: 50px;\n  width: 50px;\n  display: flex;\n  align-items: center;\n  justify-content: center;\n  padding: 8px;\n  cursor: pointer;\n  transition: all 0.3s ease-in-out;\n}\n.edit-icon-btn[_ngcontent-%COMP%]:hover {\n  background: rgba(0, 0, 0, 0.8);\n}\n.edit-icon-btn[_ngcontent-%COMP%]:hover   svg[_ngcontent-%COMP%]   path[_ngcontent-%COMP%] {\n  fill: #fff;\n}\n.preview-img[_ngcontent-%COMP%] {\n  width: 60px;\n  height: 60px;\n  object-fit: cover;\n}\n  image-cropper {\n  max-width: 200px;\n  height: 200px;\n}\n  image-cropper .cropper-container .cropper-drag-box {\n  background-color: rgb(0, 0, 0) !important;\n}\n/*# sourceMappingURL=user-profile.component.css.map */"] });
  }
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(UserProfileComponent, { className: "UserProfileComponent", filePath: "src/app/user-portal/pages/user-profile/user-profile.component.ts", lineNumber: 20 });
})();
export {
  UserProfileComponent
};
//# sourceMappingURL=chunk-GMFAMGTP.js.map
